...
class Jacobiana():
    def __init__(self, tensoes: np.ndarray, angs: np.ndarray, fases: list, anual:bool, hora:int) -> None:
        if not anual:
    ...
    def derivadas_anual(self, Gs: np.ndarray, Bs: np.ndarray, inj_pot_at_est: np.ndarray, inj_pot_rat_est: np.ndarray, hora: int, jac_dict:dict) -> dict:
        diff_angs2 = self.angs - self.matriz_angs.T
        diff_angs2 = diff_angs2.T
        diff_angs = diff_angs2[:-3, :-3]
        Gsc, Bsc = Gs[3:, 3:], Bs[3:, 3:]
        ...
        jac_dict[f"hora_{hora}"] = sp.csr_matrix(jacobiana)
        return jac_dict
