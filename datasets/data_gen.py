...
from amostragem_cargas import samplermontecarlo as smc
from amostragem_cargas import samplermontecarlo_normal as smc_norm
from amostragem_cargas import kumaraswamymontecarlo as smc_ks
from amostragem_cargas import progressBar
...
# Construção da rede no pandapower
if grid=='ober':
...
# Perfis de carga geral e GD
if grid=='ober':
    sgen_inc = 1.5 # Parâmetro para ajutar a GD
    load_inc = 1.5 # Parâmetro para ajustar a carga
    
if grid=='cigre':
    sgen_inc = 1.4 # Parâmetro para ajutar a GD
    load_inc = 1.2 # Parâmetro para ajustar a carga

    
if grid=='ober2':
    sgen_inc = 2. # Parâmetro para ajutar a GD
    load_inc = .5 # Parâmetro para ajustar a carga


# Parâmetros do algoritmo do PandaPower
algorithm = "wls"
init='flat'
...
load_dist = "normal" # Setar o tipo de distribuição da carga para amostragem
se_case = "perso"    # Nível de ruído das medidas

max_loading = 90    # Limite para nível de carga
...
""" RMSE entre o desvio de carga atual e o perfil de carga """

mape_load = pd.DataFrame(columns = np.arange(iteration))
mape_sgen = pd.DataFrame(columns = np.arange(iteration))

load_scen= pd.DataFrame(data = 0,columns = range(24), index= net.bus.index, dtype= "float64")
load_pro= pd.DataFrame(data = 0,columns = range(24), index= net.bus.index, dtype= "float64")
for t in range(24):
...
for t in progressBar(ts, prefix = 'Progress:', suffix = 'Complete', length = 50):
    ...
    pm_bus= pd.DataFrame(data = {"p_mw": 0, "q_mvar": 0}, index= net.bus.index, dtype= "float64")
    ...
    meas_bus = pd.DataFrame({"vm_pu": pf_vm[t][v_bus] + np.random.normal(loc=0., scale = np.abs(pf_vm[t][v_bus] * v_noise)),\
                             ...
                             "q_mvar": pf_q[t][p_bus] + np.random.normal(loc=0. , scale = np.abs(pf_q[t][p_bus] * meas_noise))}, index = net.bus.index, dtype= "float64")
...
# Certifica-se de que o diretório existe antes de salvar o arquivo
os.makedirs("data_"+grid, exist_ok=True)

#Salvando como .npy
np.save("data_"+grid+"/A_"+se_case+"_meas"+str(meas_case)+"(0)", A_dss)    
np.save("data_"+grid+"/B_"+se_case+"_meas"+str(meas_case)+"(0)", B_dss)
np.save("data_"+grid+"/U_"+se_case+"_meas"+str(meas_case)+"(0)", U_dss)
