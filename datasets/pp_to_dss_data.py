...
def DSSData_from_PPNet(net,pm_bus,meas_bus, meas_line, meas_error, labels): 
    ...
    """
    - Função que pega uma rede do PandaPower e a converte em uma amostra de dados DSS. 
    - Pseudomedições e localização de medições também são fornecidas para rotulagem e entrada.
    - Entrada:
        - Rede: Rede do PandaPower, com topologia detalhada, parâmetros de barras, trafos e linhas, além de geradores de carga
        
        - pm_bus = {"p_mw" : pm_p, "q_mvar": pm_q} Dataframe de pseudomedições de injeção p e q nas barras.
            - df: [index = net.bus.index, column = p_mw, q_mvar]
            
        - meas_bus = {"vm_pu": meas_v, "va_rad": meas_phi, "p_mw": meas_p, "q_mvar": meas_q}: DataFrame de medições v, phi, p e q em barras.
